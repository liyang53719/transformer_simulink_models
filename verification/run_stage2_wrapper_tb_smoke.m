...
%   accepts kv_mem_* requests and returns kv_cache_rd_* data with fixed latency.
    ...
    inports = get_root_ports(mdlName, 'Inport');
    outports = get_root_ports(mdlName, 'Outport');
    ...
    add_block('simulink/Ports & Subsystems/Model', [tbName '/dut'], ...
        'ModelName', mdlName, 'Position', [500, 180, 760, 520]);
    configure_soc_style_ddr_ref([tbName '/ddr_ref_u']);

    build_typed_sources(tbName, mdlName, inports);
    connect_dut_inputs(tbName, mdlName, inports);
