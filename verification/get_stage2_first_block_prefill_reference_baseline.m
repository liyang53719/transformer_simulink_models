...
%GET_STAGE2_FIRST_BLOCK_PREFILL_REFERENCE_BASELINE Build a real-reference baseline for first-block prefill.
    ...
    ctx = struct('Parameters', params, 'LayerIndex', layerIndex, ...
        'TokenPos', double(getFieldOr(stimulus, 'cfg_token_pos', 1)));
    ...
    for tokenIndex = 1:tokenCount
        contract = build_prefill_contract(stimulus, tokenHidden(tokenIndex), tokenResidual(tokenIndex), tokenIndex);
...
function contract = build_prefill_contract(stimulus, tokenHidden, tokenResidual, tokenIndex)
    contract = struct();
    contract.mode_decode = false;
    contract.start = tokenIndex == 1;
    contract.eos_in = logical(getScalarOr(stimulus, 'eos_in', tokenIndex, 0));
    contract.in_valid = true;
    contract.out_ready = logical(getScalarOr(stimulus, 'out_ready', tokenIndex, 1));
    ...
    contract.cfg_token_pos = double(getFieldOr(stimulus, 'cfg_token_pos', 1)) + tokenIndex - 1;
