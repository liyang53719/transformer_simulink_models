...
%   projection consumes the SwiGLU result.
    ...
    signalSpecs = {
        ...
        struct('block', 'swiglu_valid_gate_z1', 'port', 1, 'name', 'ffn_swiglu_valid_gate_z1'), ...
        struct('block', 'swiglu_valid_gate_z2', 'port', 1, 'name', 'ffn_swiglu_valid_gate_z2'), ...
        struct('block', 'down_valid_z', 'port', 1, 'name', 'ffn_down_valid_z'), ...
        struct('block', 'down_valid_z2', 'port', 1, 'name', 'ffn_down_valid_z2'), ...
        struct('block', 'down_pair_valid', 'port', 1, 'name', 'ffn_down_pair_valid'), ...
    ...
    simOut = sim(tbName, 'StopTime', '8', 'SaveOutput', 'on', 'OutputSaveName', 'yout', ...
    ...
    swigluValidGateZ1 = extract_logged_signal(logsout, 'ffn_swiglu_valid_gate_z1');
    swigluValidGateZ2 = extract_logged_signal(logsout, 'ffn_swiglu_valid_gate_z2');
    downValidZ = extract_logged_signal(logsout, 'ffn_down_valid_z');
    downValidZ2 = extract_logged_signal(logsout, 'ffn_down_valid_z2');
    downPairValid = extract_logged_signal(logsout, 'ffn_down_pair_valid');
    ...
    result.swiglu_gate_d1_ok = has_single_cycle_delay(swigluValidZ, swigluValidGateZ1);
    result.swiglu_gate_d2_ok = has_single_cycle_delay(swigluValidGateZ1, swigluValidGateZ2);
    result.down_req_d1_ok = has_single_cycle_delay(swigluValidGateZ2, downValidZ);
    result.down_req_d2_ok = has_single_cycle_delay(downValidZ, downValidZ2);
    result.down_pair_align_ok = all(abs(double(downPairValid(:)) - double(downValidZ2(:))) < 1e-9);
    result.down_valid_d1_ok = has_single_cycle_delay(downPairValid, downPairValidZ1);
    result.down_valid_d2_ok = has_single_cycle_delay(downPairValidZ1, downPairValidZ2);
    result.valid_chain_order_ok = respects_chain_order(pairValid, pairValidZ, swigluValidZ, swigluValidGateZ1, swigluValidGateZ2, downValidZ, downValidZ2, downPairValid, downPairValidZ1, downPairValidZ2);
    result.pass = result.pair_valid_seen && result.pair_valid_delay_ok && ...
        result.swiglu_valid_delay_ok && result.swiglu_gate_d1_ok && ...
        result.swiglu_gate_d2_ok && result.down_req_d1_ok && ...
        result.down_req_d2_ok && result.down_pair_align_ok && ...
        result.down_valid_d1_ok && ...
    ...
    if result.pass
        ...
        fprintf(['  pair_seen=%d pair_d1=%d swiglu_d1=%d gate_d1=%d gate_d2=%d ' ...
            'down_req_d1=%d down_req_d2=%d down_align=%d down_d1=%d down_d2=%d chain_order=%d\n'], ...
            result.pair_valid_seen, result.pair_valid_delay_ok, ...
            result.swiglu_valid_delay_ok, result.swiglu_gate_d1_ok, ...
            result.swiglu_gate_d2_ok, result.down_req_d1_ok, ...
            result.down_req_d2_ok, result.down_pair_align_ok, result.down_valid_d1_ok, ...
            result.down_valid_d2_ok, ...
            result.valid_chain_order_ok);
...
function yes = respects_chain_order(pairValid, pairValidZ, swigluValidZ, swigluValidGateZ1, swigluValidGateZ2, downValidZ, downValidZ2, downPairValid, downPairValidZ1, downPairValidZ2)
    idx0 = first_active_index(pairValid);
    idx1 = first_active_index(pairValidZ);
    idx2 = first_active_index(swigluValidZ);
    idx3 = first_active_index(swigluValidGateZ1);
    idx4 = first_active_index(swigluValidGateZ2);
    idx5 = first_active_index(downValidZ);
    idx6 = first_active_index(downValidZ2);
    idx7 = first_active_index(downPairValid);
    idx8 = first_active_index(downPairValidZ1);
    idx9 = first_active_index(downPairValidZ2);
    yes = ~isnan(idx0) && ~isnan(idx1) && ~isnan(idx2) && ~isnan(idx3) && ~isnan(idx4) && ...
        ~isnan(idx5) && ~isnan(idx6) && ~isnan(idx7) && ~isnan(idx8) && ~isnan(idx9) && ...
        idx0 <= idx1 && idx1 <= idx2 && idx2 <= idx3 && idx3 <= idx4 && ...
        idx4 <= idx5 && idx5 <= idx6 && idx6 <= idx7 && idx7 <= idx8 && idx8 <= idx9;
end
