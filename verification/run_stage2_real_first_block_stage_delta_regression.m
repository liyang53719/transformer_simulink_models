...
function specs = append_weight_lane_specs(moduleName, prefix)
    ...
    specs = { ...
        struct('scope', 'mdl', 'block', [moduleName '/' prefix '_req_addr_u8'], 'port', 1, 'name', [baseName '_req_addr_u8'], 'setName', false), ...
        struct('scope', 'mdl', 'block', [moduleName '/' prefix '_req_addr_z'], 'port', 1, 'name', [baseName '_req_addr_z'], 'setName', false), ...
...
function laneSummaries = build_lane_summaries(stageResults)
    ...
    laneSummaries = repmat(struct( ...
        ...
        'req_addr_active', false, ...
        'req_addr_delay_active', false, ...
    ...
    for i = 1:size(laneTable, 1)
        ...
        laneSummaries(i).req_addr_active = any_nonzero(stageResults, [key '_req_addr_u8']);
        laneSummaries(i).req_addr_delay_active = any_nonzero(stageResults, [key '_req_addr_z']);
...
function print_lane_summaries(laneSummaries)
    for i = 1:numel(laneSummaries)
        fprintf(['  lane=%s src_data_changed=%d src_valid_active=%d src_addr_active=%d ' ...
            'req_addr_active=%d req_addr_delay_active=%d ddr_data_changed=%d ddr_valid_active=%d ' ...
            ...
            laneSummaries(i).req_addr_active, ...
            laneSummaries(i).req_addr_delay_active, ...
