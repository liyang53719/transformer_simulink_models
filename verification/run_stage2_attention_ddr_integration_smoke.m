...
%   4) KV writeback reaches the external DDR wrapper path.
    ...
    attnQReqAddr = extract_signal(yout, 'tb_attn_q_req_addr');
    attnKReqAddr = extract_signal(yout, 'tb_attn_k_req_addr');
    attnVReqAddr = extract_signal(yout, 'tb_attn_v_req_addr');
    ...
    result.pass = result.attn_q_req_seen && result.attn_k_req_seen && result.attn_v_req_seen && ...
        ...
        result.attn_rsp_data_distinct && result.attn_q_rsp_matches_req && ...
        result.attn_k_rsp_matches_req && result.attn_v_rsp_matches_req && ...
    ...
    if result.pass
        ...
        fprintf(['  attn_q_req=%d attn_k_req=%d attn_v_req=%d attn_q_rsp=%d attn_k_rsp=%d attn_v_rsp=%d ' ...
            'attn_q_data=%d attn_k_data=%d attn_v_data=%d attn_data_distinct=%d q_match=%d k_match=%d v_match=%d rd_rsp_valid=%d rd_rsp_data=%d ' ...
            ...
            result.attn_q_rsp_matches_req, result.attn_k_rsp_matches_req, result.attn_v_rsp_matches_req, ...
            result.rd_rsp_valid_seen, result.rd_rsp_data_nonzero, ...
            result.out_hidden_nonzero, result.kv_wr_valid_seen, ...
            result.kv_wr_en_seen, result.kv_wr_data_nonzero);
...
function yes = matches_tagged_response(reqAddr, rspData, signature)
