...
- 可视化外层 TB 模型：`simulink/models/qwen2_block_top_wrapper_tb.slx`。这个模型把 `qwen2_block_top` 作为 DUT，并在外层显式放置 SoC 风格的 `ddr_ref_u` responder，作用上更接近 `simulink/ref/soc_image_rotation_fpga.slx` 的“DUT + DDR 环境”组织方式。
