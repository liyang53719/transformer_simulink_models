...
- `run_stage2_real_layer0_attention_weight_smoke`：真实参数联调检查。它从 `matlab_ref/cache` 中加载 Qwen2 第一层真实权重缓存，把 `input_layernorm/self_attn_q_proj/self_attn_k_proj/self_attn_v_proj/mlp_*_proj` 压缩成 per-lane 响应签名，灌入 wrapper `weight_ref_u`，然后复用 attention+DDR 集成检查验证真实 layer0 派生签名已经进入外部权重响应链。
