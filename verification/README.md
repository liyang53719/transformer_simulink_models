...
- `run_stage2_attention_ddr_integration_smoke`：主线联调 smoke。复用同一个 wrapper TB，同时检查 attention 权重请求有效位是否发出、wrapper 外部 `weight_ref_u` 是否按请求地址返回带页签名的 attention 权重响应数据、DDR 读响应是否返回、`out_hidden` 是否产生非零结果，以及 KV 写回是否落到外层 DDR 通路上，用来覆盖“attention 细节 + DDR 联调”这条主线。
...
- `run_stage2_smoke_suite_fast`：快速套件，复用建模结果串行执行 decode、`kv_cache_if_u` 边界、prefill-attention、wrapper TB、attention+DDR 集成 smoke、rd/wr smoke，并附加一组非默认 `KvAddressConfig`（`rd_base/wr_base/stride_bytes/decode_burst_len`）覆盖。
