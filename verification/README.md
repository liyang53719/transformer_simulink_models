...
- `run_stage2_attention_ddr_integration_smoke`：主线联调 smoke。复用同一个 wrapper TB，同时检查 attention 权重请求有效位是否发出、attention 请求地址与 `weight_ref_u` 返回的带页签名响应数据是否精确匹配、DDR 读响应是否返回、`out_hidden` 是否产生非零结果，以及 KV 写回是否落到外层 DDR 通路上，用来覆盖“attention 细节 + DDR 联调”这条主线。
