...
- 可视化外层 TB 模型：`simulink/models/qwen2_block_top_wrapper_tb.slx`。这个模型把 `qwen2_block_top` 作为 DUT，并在外层显式放置 SoC 风格的 `ddr_ref_u` responder；`ddr_ref_u` 当前已拆成可见的 `Input Read Memory` / `Output Write Memory` 两段结构，组织方式上更接近 `simulink/ref/soc_image_rotation_fpga.slx` 的“DUT + DDR 环境”风格。
...
- `run_stage2_smoke_suite_fast`：快速套件，复用建模结果串行执行 decode、`kv_cache_if_u` 边界、prefill-attention、wrapper TB、rd/wr smoke，并附加一组非默认 `KvAddressConfig`（`rd_base/wr_base/stride_bytes/decode_burst_len`）覆盖。
