...
- `run_stage2_smoke_suite_fast`：快速套件，复用建模结果串行执行 decode、`kv_cache_if_u` 边界、prefill-attention、wrapper TB、KV banking pipeline、attention pipeline、FFN pipeline、attention+DDR 集成 smoke、rd/wr smoke，并附加一组非默认 `KvAddressConfig`（`rd_base/wr_base/stride_bytes/decode_burst_len`）覆盖。
